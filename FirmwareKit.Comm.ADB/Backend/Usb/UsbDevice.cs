...
/// <summary>
/// Abstract base class for ADB USB devices, implementing the buffered transport interface.
/// <para>ADB USB 设备的抽象基类，实现缓冲传输接口。</para>
/// </summary>
public abstract class UsbDevice : IAdbTransport
{
    /// <summary>
    ...
    /// <summary>
    /// Gets or sets the stable identity key for the device, used to reopen the same
    /// physical device after a mode switch (adb → fastboot → EDL) where VID/PID change.
    /// <para>获取或设置设备的稳定标识键，用于模式切换（adb → fastboot → EDL）导致
    /// VID/PID 变化后重开同一物理设备。</para>
    /// </summary>
